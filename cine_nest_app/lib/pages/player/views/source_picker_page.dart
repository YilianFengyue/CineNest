...
class _SourcePickerPageState extends State<SourcePickerPage> {
  ...
  void _readArgs() {
    ...
    if (url != null && url.trim().isNotEmpty) {
      _initialUrl = url.trim();
    }
    final sourceName = args is Map ? args['sourceName']?.toString() : null;
    if (sourceName != null && sourceName.trim().isNotEmpty) {
      _initialSourceName = sourceName.trim();
    }
    _title = _movieName;
  }

  Future<void> _playInitialUrl() async {
    final playUrl = _initialUrl;
    if (playUrl == null || playUrl.isEmpty) return;
    final source = VideoSource(
      id: 'local:${Uri.encodeComponent(playUrl)}',
      name: _initialSourceName ?? _movieName,
      quality: 'PC local',
      type: SourceType.web,
      playUrl: playUrl,
    );
    ...
    try {
      ...
      setState(() {
        _switching = false;
        _error = e.toString();
        _statusMessage = 'PC 本地视频播放失败，可重试或检查后端连接';
      });
    }
  }
  ...
  bool _isDirectVideo(String url) {
    ...
    return lower.contains('.m3u8') ||
        ...
        lower.contains('.webm') ||
        lower.contains('.avi') ||
        lower.contains('/api/local-videos/stream/') ||

...
class _TasteDnaPageState extends State<TasteDnaPage> {
  ...
  Future<void> _generateAvatar() async {
    ...
    try {
      ...
      await _load();
      if (!mounted) return;
      if (avatar.warning != null && avatar.warning!.isNotEmpty) {
        setState(() => _warning = avatar.warning);
        _pollAvatarUpdate(oldAvatarUrl);
      }
      if (avatar.avatarUrl.isNotEmpty) {
      ...
      try {
        ...
        if (changed) {
          ...
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('New avatar is ready.')));
          break;
        }
      } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : dna == null
          ? _ErrorView(
              error: _error ?? 'Taste DNA load failed.',
              onRetry: _load,
            )
          : ListView(
...
class _WarningBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: Padding(
        padding: const EdgeInsets.all(12),
        child: Text(message, style: TextStyle(color: cs.onTertiaryContainer)),
      ),
    );
  }
}

...
class PosterController extends GetxController {
  ...
  Future<void> _fetchReal(
    ...
    if (res.statusCode == 200 && data is Map) {
      ...
      if (blocks is! List || blocks.isEmpty) {
        throw StateError('海报接口返回空 blocks');
      }
      ...
      final message = data is Map
          ...
          : null;
      throw StateError(message?.isNotEmpty == true ? message! : '海报数据异常');
    }
  }
  ...
  void _applySpec(Map<String, dynamic> spec) {
    ...
    subtitle = (spec['subtitle'] as String?)?.trim().isNotEmpty == true
        ...
        : banner.str('subtitle');
    style =
        (spec['style'] as String?) ?? banner.data['style'] as String? ?? 'warm';
    ...
    body.value = blocks
        .where((b) => b.type != ContentBlockType.banner)
        .toList();

    final actions =
        (spec['actions'] as List?)
            ?.whereType<Map>()
